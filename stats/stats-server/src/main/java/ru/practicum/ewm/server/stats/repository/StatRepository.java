...
public interface StatRepository extends JpaRepository<StatData, Long> {
    ...
    @Query("SELECT new ru.practicum.ewm.server.stats.model.ViewStats(s.appName, s.uri, COUNT(s.ip) as hits) " +
            "FROM StatData AS s " +
    ...
    @Query("SELECT new ru.practicum.ewm.server.stats.model.ViewStats(s.appName, s.uri, COUNT(DISTINCT s.ip) as hits) " +
            "FROM StatData AS s " +
            "WHERE s.created BETWEEN :start AND :end " +
            "GROUP BY s.appName, s.uri")
